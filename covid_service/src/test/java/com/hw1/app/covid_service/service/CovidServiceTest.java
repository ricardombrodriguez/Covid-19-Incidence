...
public class CovidServiceTest {
    ...
    @Test
    void testGetCountryIntervalHistoryHit() throws URISyntaxException, IOException, ParseException, java.text.ParseException {
        ...
        Mockito.when(cache.getRequestStatistics(anyString(),any(Date.class),any(Date.class),anyInt())).thenReturn(request);

        Request actualRequest = covidService.getCountryIntervalHistory(country,initial,end);

        Mockito.verify(cache, times(1)).getRequestStatistics(anyString(),any(Date.class),any(Date.class),anyInt());
        assertThat(actualRequest).isEqualTo(request);
        assertThat(actualRequest.getCacheStatus()).isEqualTo(CacheStatus.HIT);

    }


    @Test
    void testGetCountryIntervalHistoryMiss() throws URISyntaxException, IOException, ParseException, java.text.ParseException {
        ...
        Request request = new Request(id, end, country, 365, time, CacheStatus.MISS, statistics);

        Mockito.when(cache.getRequestStatistics(anyString(),any(Date.class),any(Date.class),anyInt())).thenReturn(null);
        Mockito.when(rapidApiResolver.getCountryIntervalHistory(anyString(),any(Date.class),any(Date.class))).thenReturn(request);

        Request actualRequest = covidService.getCountryIntervalHistory(country,initial,end);

        Mockito.verify(cache, times(1)).getRequestStatistics(anyString(),any(Date.class),any(Date.class),anyInt());
        Mockito.verify(rapidApiResolver, times(1)).getCountryIntervalHistory(anyString(),any(Date.class),any(Date.class));
        Mockito.verify(cache, times(1)).storeRequestStatistics(anyString(), any(Request.class));
        assertThat(actualRequest).isEqualTo(request);
        assertThat(actualRequest.getCacheStatus()).isEqualTo(CacheStatus.MISS);
